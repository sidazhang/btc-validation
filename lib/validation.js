...
me.tx = function(tx, prevOuts, callback) {
  ...
  async.series([
    ...
    function(callback) {
      ...
      var raw = tx.serialize().length
      if (!(raw < MAX_BLOCK_SIZE) && raw > 100) {
        return callback(new Error('Tx size must be smaller than max block size and greater than 100 bytes'), false)
      }
      callback(null, true)
    },
    function(callback) {
    ...
    function(callback) {
      ...
      // todo: implement isStandard check in btc-transaction
      callback(null, true)
    },
    function(callback) {
